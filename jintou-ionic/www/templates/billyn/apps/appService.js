...
(function () {
	...
	function AppService($resource, User, $q, Util, $http, BNut, $rootScope, BPermit) {
		...
		service.create = function (spaceId, appName) {
			...
			return $http.get('/app/apps/config.json').then(function (res) {
		...
		*/
		...
		service.getAppConfig = function (appData) {
			...
			return model.find(appData).$promise.then(function (app) {
				return $http.get('/templates/billyn/apps/config.json').then(function (res) {
					var appConfig = res.data;
